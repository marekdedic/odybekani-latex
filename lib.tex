...
\newcommand{\listsongname}{Obsah}
\newlistof{song}{los}{\listsongname}
\cftpagenumbersoff{song}

\newsavebox{\songbox}
\newcommand\songmeasurebox{\dimexpr\ht\songbox+\dp\songbox\relax}

\newcommand{\songentry}[2]{%
	\refstepcounter{song}
	\addcontentsline{los}{song}{\protect #1 \hfill #2}
}

\newenvironment{song}[4][]{\directlua{startrecording("#2", "#3", "#1", "#4")}\songentry{#3}{#2}}{%
...
\newpagestyle{songstyle}{
	\footrule
	\sethead[]
		[\begin{tabular}{@{}>{\raggedleft\small\arraybackslash}p{\textwidth}@{}}\multicolumn{1}{c}{\centering \songnamefont \topshortmark\songnamemark} \\\midrule {\songauthorfont \topshortmark\songauthormark \hspace{0.5cm} \null}\end{tabular}]
		[]{}
...
% Page geometry
\geometry{a5paper,twoside,top=70pt,bottom=50pt,inner=55pt,outer=55pt,headheight=25pt,headsep=20pt,footskip=20pt}
